...
use crate::what_ruby_reported;

actions!(
    ruby_diagnostics,
    [
        /// Asks ruby -c what is wrong with every open Ruby file and shows
...
const SERVER_ID: language::LanguageServerId = language::LanguageServerId(usize::MAX - 1018);
...
const SOMETHING_WRONG: i32 = 1;

/// Reports what ruby -c says about every open Ruby file in one workspace,
/// after each save -- but only where no language server is doing it already.
///
/// That condition is the whole design. A project with a Ruby server running
...
fn check_one(
    ...
    cx.spawn(async move |_, cx| {
            ...
            .await
        {
            Ok(found) => found,
            Err(error) => {
                // A machine with no ruby -c is the ordinary case rather than a
        ...
        if let Err(error) = show(&project, path, found, cx) {
            log::warn!("showing what ruby -c said: {error:#}");
        }
    })
    .detach();
}
...
async fn ask(path: &Path) -> Result<Vec<lsp::Diagnostic>> {
    ...
    let tool = which::which("ruby").context("ruby is not on PATH")?;
    ...
    if !matches!(asked.status.code(), Some(NOTHING_WRONG | SOMETHING_WRONG)) {
        return Err(anyhow!("ruby could not read {}", path.display()));
    }
    // Read from both: ruby writes the syntax report to stderr and `Syntax OK`
    // to stdout, and reading only one of the two would either lose every fault
    // or never see a clean file.
    ...
    Ok(what_ruby_reported(&output, named, &text))
}
