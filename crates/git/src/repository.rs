...
#[derive(Debug, Clone)]
pub enum RawDiff {
    /// What one commit changed, against its first parent, and whether to
    /// ignore the repository's shallow-clone boundary when answering.
    Of(String, bool),
...
pub trait GitRepository: Send + Sync {
    ...
    fn load_commit(
        ...
        self.load_raw_diff(RawDiff::Of(commit, ignore_shallow_boundary), cx)
    }
...
impl GitRepository for RealGitRepository {
    ...
    fn load_raw_diff(&self, what: RawDiff, cx: AsyncApp) -> BoxFuture<'_, Result<CommitDiff>> {
        ...
        cx.background_spawn(async move {
            if let RawDiff::Of(commit, ignore_shallow_boundary) = &what
            ...
            let mut command = match &what {
                ...
                // history draws it as.
                RawDiff::Of(_, _) => git.build_command(&[
                    "show",
                    "--format=",
                    "-z",
                    "--no-renames",
            ...
            match &what {
                RawDiff::Of(commit, _) => command.arg(commit).arg("--"),
                RawDiff::Between(from, to) => command.arg(from).arg(to).arg("--"),
            };
