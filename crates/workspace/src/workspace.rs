...
use gpui::{
    Action, AnimationExt as _, AnyElement, AnyEntity, AnyView, AnyWeakView, App, AppContext,
    AsyncApp, AsyncWindowContext, Axis, Bounds, ClipboardItem, Context, CursorStyle, Decorations,
        ...
        window.spawn(cx, async move |cx| {
            ...
            let pane = if reveal_if_open {
                workspace
                    ...
                    .unwrap_or_else(|| requested_pane.clone())
            } else {
                requested_pane.clone()
            };

            // The placeholder tab lives on `requested_pane`. When the file
            // turned out to already be open elsewhere, the final `pane`
            // differs and the placeholder cannot be reused as the slot to
            // replace -- it is simply dropped once the real item is open.
            ...
            let placeholder = if pane == requested_pane {
                placeholder
            } else {
                None
            };
