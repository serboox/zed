...
mod tests {
    ...
    use gpui::{KeyBinding, TestAppContext, UpdateGlobal as _, VisualTestContext};
    ...
    async fn init_selector_test_with_capabilities(
        ...
        let app_state = cx.update(|cx| {
            ...
            settings::init(cx);
            // `settings::init` brings back the shipped defaults, under which no
            // language server starts until asked; these tests need their fake
            // server to start as soon as the file opens.
            settings::SettingsStore::update_global(cx, |store, cx| {
                store.update_user_settings(cx, |settings| {
                    settings.global_lsp_settings.get_or_insert_default().start =
                        Some(project::project_settings::LanguageServerStart::Automatically);
                });
            });
            super::init(cx);
            editor::init(cx);
