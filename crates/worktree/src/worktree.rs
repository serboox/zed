    ...
    fn load_file(&self, path: &RelPath, cx: &Context<Worktree>) -> Task<Result<LoadedFile>> {
        ...
        cx.background_spawn(async move {
            ...
            let (text, line_ending, encoding, has_bom) =
                decode_file_text_to_rope(fs.as_ref(), &abs_path, read_progress.as_deref()).await?;
            ...
            .map(|child_abs_path| {
                let should_stat = child_abs_path
                    ...
                    .is_some_and(|name| {
                        !self
                            .settings
                            .is_path_excluded(&job.path.join(name.as_ref()))
                    });
...
fn read_file_header(
    ...
    while header.len() < FILE_ANALYSIS_BYTES {
        ...
        if let Some(progress) = progress {
            progress.set_bytes_read(header.len() as u64);
        }
    }
...
pub async fn decode_file_text(
    ...
    let (file_first_bytes, reached_eof) = read_file_header(&mut *file, abs_path, progress)?;
    ...
    if !reached_eof {
        read_file_to_end(&mut *file, &mut content, abs_path, progress).await?;
    }
    if let Some(progress) = progress {
...
pub async fn decode_file_text_to_rope(
    ...
    let (prefix, reached_eof) = read_file_header(&mut *file, abs_path, progress)?;
    ...
    if bom_encoding.is_none()
        ...
        && let Some((rope, line_ending)) =
            stream_utf8_into_rope(&mut *file, prefix, reached_eof, abs_path, progress).await?
    {
        if let Some(progress) = progress {
            progress.mark_finished();
        }
    ...
    let (mut text, encoding, has_bom) = decode_file_text(fs, abs_path, progress).await?;
    ...
    loop {
        ...
        while !eof && pending.len() < STREAM_BLOCK_BYTES {
            ...
            if n == 0 {
                ...
                bytes_read += n as u64;
                if let Some(progress) = progress {
                    progress.set_bytes_read(bytes_read);
                }
            }
        }
...
mod tests {
    ...
    async fn stream(bytes: &[u8]) -> Option<(String, LineEnding)> {
        ...
        stream_utf8_into_rope(&mut reader, Vec::new(), false, Path::new("test"), None)
            .await
            .unwrap()
    ...
    fn test_file_reading_yields_between_blocks() {
        ...
        assert!(
            read_file_to_end(&mut reader, &mut content, Path::new("test"), None)
                .now_or_never()
                .is_none()
        );
        assert_eq!(reader.position(), STREAM_BLOCK_BYTES as u64);
