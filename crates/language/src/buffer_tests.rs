...
"#,
    ...
    // Like the colorization test above: the fixture outgrows the 10ms synchronous
    // parse budget on a loaded machine, and the assertions need the tree.
    let buffer = cx.new(|cx| {
        let mut buffer = Buffer::local(text.clone(), cx);
        buffer.set_sync_parse_timeout(Some(Duration::from_secs(30)));
        buffer.with_language(c_lang(), cx)
    });
    let snapshot = buffer.read(cx).snapshot();
    assert_has_syntax_errors(&snapshot);
