...
impl Language {
    ...
    pub fn highlight_text_with_theme(
        self: &Arc<Self>,
        text: &Rope,
        range: Range<usize>,
        theme: &SyntaxTheme,
        ...
        let highlight_map = build_highlight_map(highlights_config.query.capture_names(), theme);
        self.compute_highlight_runs(grammar, text, &highlight_map)
            .into_iter()
            .filter(|(run_range, _)| run_range.start < range.end && run_range.end > range.start)
            .map(|(run_range, highlight_id)| {
                (
    ...
    fn compute_resolved_highlights(
        ...
        let runs = self.compute_highlight_runs(grammar, text, &highlight_map);
        ResolvedHighlights {
