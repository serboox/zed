...
use gpui::{
    ...
    MouseButton, MouseDownEvent, Pixels, Point, PromptLevel, ScrollStrategy, ScrollWheelEvent,
...
pub struct GitPanel {
    ...
    /// The row the pointer is over, and the rows lit because of it. The set is
    /// worked out when the pointer moves rather than every frame: it does not
    /// change while the pointer sits still, and the list repaints constantly.
...
impl GitPanel {
    ...
    fn select_previous(
        ...
        if self.active_tab == GitPanelTab::History {
            self.select_previous_history_entry(window, cx);
            return;
        }
    ...
    fn select_next(&mut self, _: &menu::SelectNext, window: &mut Window, cx: &mut Context<Self>) {
        if self.active_tab == GitPanelTab::History {
            self.select_next_history_entry(window, cx);
            return;
        }
                                        ...
                                        .map(|(ix, (entry, data))| {
                                            ...
                                            v_flex()
                                                ...
                                                .on_hover({
                                                    let git_panel = git_panel.clone();
                                                    move |over, _, cx| {
                                                        let row = over.then_some(index);
                                                        git_panel
                                                            .update(cx, |panel, cx| {
                                                                if *over
                                                                    || panel.hovered_history_entry
                                                                        == Some(index)
                                                                {
                                                                    panel
                                                                        .light_history_kin(row, cx);
                                                                }
                                                            })
                                                            .ok();
                                                    }
