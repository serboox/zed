...
impl MarkdownStyle {
    ...
    fn with_preview_overrides(mut self, colors: &theme::ThemeColors) -> Self {
        ...
        self.paragraph_line_height = Some(relative(1.5));
...
impl MarkdownElement {
    ...
    fn push_markdown_paragraph(
        ...
        let mut paragraph = div().when(!self.style.height_is_multiple_of_line_height, |el| {
            el.mb(self.style.paragraph_spacing).line_height(
                self.style
...
fn apply_heading_style(
        ...
        && border_levels[level_index]
    {
        heading = match level {
            ...
            _ => heading.pb_1().border_b_1().border_color(border_color),
        };
    }
...
impl MarkdownElementBuilder {
    ...
    fn push_text(&mut self, text: &str, source_range: Range<usize>) {
        ...
        if let Some(language) = self.code_block_stack.last().and_then(Option::as_ref)
            && self.code_block_highlights.contains_key(&source_range.start)
        {
            // Highlight ids index the theme they were resolved against, and this
            // renderer paints with its own syntax theme rather than the app's.
            let runs = language.highlight_text_with_theme(
                &Rope::from(text),
                0..text.len(),
                &self.syntax_theme,
            );
